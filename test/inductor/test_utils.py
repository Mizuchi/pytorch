...
class TestUtils(TestCase):
    ...
    def test_flops_fx(self):
        def create_fx_node(aten: torch._ops.OpOverloadPacket, args, kwargs) -> tuple[torch.fx.Node, torch.fx.Node]:
            node1 = torch.fx.Node(
        ...
        with V.set_fake_mode(torch._subclasses.FakeTensorMode(allow_non_fake_inputs=True)):
            trues = [
                (torch.ops.aten.addmm, (torch.Tensor(4, 4), torch.Tensor(4, 5), torch.Tensor(5, 4)), {}),
                (torch.ops.aten.bmm, (torch.Tensor(10, 4, 5), torch.Tensor(10, 5, 4)), {}),
                (torch.ops.aten.mm, (torch.Tensor(2, 3), torch.Tensor(3, 2)), {}),
                (torch.ops.aten.convolution, (torch.Tensor(2, 3, 3), torch.Tensor(2, 2, 2), torch.Tensor(2), (1, 1), (0, 0), (1, 1), True, (0, 0), 1), {}),
                (torch.ops.aten._convolution, (torch.Tensor(2, 2, 2), torch.Tensor(2, 2, 2), torch.Tensor(2), (1,), (0,), (1,), True, (0,), 1, False, True, False), {}),
            ]
            # we don't support pointwise ops
            falses = [
                (torch.ops.aten.add, (torch.Tensor(1, 2, 3), torch.Tensor(1, 2, 3)), {}),
                (torch.ops.aten.mul, (torch.Tensor(1, 2, 3), torch.Tensor(1, 2, 3)), {}),
            ]
            for t, args, kwargs in trues:
                fx_node_1, fx_node_2 = create_fx_node(t, args, kwargs)
                self.assertTrue(countable_fx(fx_node_1), f"Expected true {t}: {fx_node_1}")
                self.assertTrue(countable_fx(fx_node_2), f"Expected true {t}: {fx_node_2}")
            ...
            for f, args, kwargs in falses:
                ...
                self.assertFalse(countable_fx(fx_node_1), f"Expected false {f}: {fx_node_1}")
                self.assertFalse(countable_fx(fx_node_2), f"Expected false {f}: {fx_node_2}")
